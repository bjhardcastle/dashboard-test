...
def get_unit_location_query_df(
    filter_area: str, 
    filter_type: Literal['starts_with', 'contains', "contains_any"] = 'starts_with',
    ...
    location_expr = getattr(location_col.str, filter_type)(filter_area if filter_type != "contains_any" else [v.strip() for v in filter_area.split(",")])
...
filter_area = pn.widgets.TextInput(name='Search brain area', value='MOs,ACA', styles={'font-weight': 'bold'})
